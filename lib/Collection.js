...
Collection.prototype._addItems = function(representation) {
    ...
    if (this.transformFunction) {
        ...
        this.items.forEach(function(elm, index) {
            items.push(this.transformFunction(elm));
        });
    } else {
...
Collection.prototype._addLinks = function(representation) {
    ...
    if (this.isPaginated) {
        ...
        if (this.total != null) {
            var offset = Math.floor(this.total / this.limit) * this.limit;
            links.add('last', { offset: offset, limit: this.limit });
        }
    }
